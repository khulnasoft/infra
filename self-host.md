# Self-hosting KHULNASOFT on Google Cloud
...
5. [Create a storage bucket in Google Cloud](https://cloud.google.com/storage/docs/creating-buckets). This is the source of truth for the terraform state: Go to `console.cloud.google.com` -> Storage -> Create Bucket -> Bucket name: `khulnasoft-terraform-state` -> Location: `US` -> Default storage class: `Standard` -> Location type: `Multi-region` -> Bucket location: `US` -> Create
...
- khulnasoft-cloudflare-api-token
- khulnasoft-postgres-connection-string
...
15. Either run 
    - `make prep-cluster` in `packages/shared` to create an initial user, etc. (You need to be logged in via [`khulnasoft` CLI](https://www.npmjs.com/package/@khulnasoft/cli)). It will create a user with same information (access token, api key, etc.) as you have in KHULNASOFT. 
    - You can also create a user in database, it will automatically also create a team, an API key and an access token. You will need to build template(s) for your cluster. Use [`khulnasoft` CLI](https://www.npmjs.com/package/@khulnasoft/cli?activetab=versions)) and run `KHULNASOFT_DOMAIN=<your-domain> khulnasoft template build`.
...
```js
import { Sandbox } from "@khulnasoft/sdk";
...
```sh
KHULNASOFT_DOMAIN=<your-domain> khulnasoft <command>
```

#### Dashboard

You can also set your domain in the dashboard at [Developer settings](https://khulnasoft.dev/dashboard?tab=developer)
...
If any problems arise, open [a Github Issue on the repo](https://github.com/khulnasoft/infra/issues) and we'll look into it.

---

### Building Firecracker and UFFD from source

KHULNASOFT is using [Firecracker](https://github.com/firecracker-microvm/firecracker) for Sandboxes.
...
- You will still have to copy `envd-v0.0.1` from public bucket by running the command bellow or you can build it from [this commit](https://github.com/khulnasoft/infra/tree/703da3b2b8ef4af450f9874228e7406bdfc75d4a)

```
gsutil cp -r gs://khulnasoft-prod-public-builds/envd-v0.0.1 gs://$(GCP_PROJECT_ID)-fc-env-pipeline/envd-v0.0.1
```
