...
import (
	...
	"github.com/khulnasoft/infra/packages/shared/pkg/models/accesstoken"
	"github.com/khulnasoft/infra/packages/shared/pkg/models/team"
)
...
func main() {
	...
	configPath := filepath.Join(homeDir, ".khulnasoft", "config.json")
	...
	// Create team
	t, err := database.Client.Team.Create().SetEmail(email).SetName("KHULNASOFT").SetID(teamUUID).SetTier("base_v1").Save(ctx)
	if err != nil {
		panic(err)
	}
	...
	// Create template
	data, err = os.ReadFile("khulnasoft.toml")
